...
public class WorkorderBuilder
{
    ...
    public static Uri BuildWorkOrdersExist(string[] workOrderIds)
    {
        var queryParams = new Dictionary<string, string?>
        {
            { "api-version", "v1" },
            { "work-order-ids-any-of", string.Join(",", workOrderIds) }
        };
        return new Uri(QueryHelpers.AddQueryString("work-order-types", queryParams), UriKind.Relative);
    }
}